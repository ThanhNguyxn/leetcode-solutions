...
int main(){
    ...
    // number of ways to choose > y from x items
    auto sum_greater = [&](int x, int y)->int{
        if(y >= x) return 0;
        int ans = pow2[x];
        for(int i = 0; i <= y; i++){
            ans = submod(ans, C(x, i));
        }
        return ans;
    };

    auto get_required = [&](long long x){
        vector<int> req(LOG, 0);
        int cnt = 0;
        for(int i = LOG - 1; i >= 0; i--){
            if((x >> i) & 1LL){
                int idx = i + cnt;
                // idx is guaranteed < LOG by problem constraints (x <= 1e18)
                req[idx]++;
                cnt++;
            }
        }
    ...
    while(m--){
        ...
        if(t == 1){
            ...
            for(int i = LOG - 1; i >= 0; i--){
                cnt_less -= freq[i]; // now cnt_less = total of exponents < i
